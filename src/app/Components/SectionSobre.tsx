import "../../Assets/css/components/sobre.css";


export default function Sobre() {
  return (
    <>
      <div className="Container-sobre" id='sobre'>
        <div className="title">
          <span>SOBRE NOIS</span>
        </div>
        <div className="box-info">
          <ul>
            <li>
              O Arboriza, Itaboraí! é um movimento cidadão independente que
              promove a arborização urbana responsável e a sustentabilidade na
              cidade.
            </li>
            <li id='middleOne'>
              Atua com base técnica, participação social e educação ambiental,
              incentivando plantio adequado, escolha correta de espécies e
              valorização da infraestrutura verde.
            </li>
            <li id='middleTwo'>
              Realiza ações práticas, orientação à população e fiscalização
              cidadã das políticas ambientais.
            </li>
            <li>
              É mantido por doações e voluntariado, garantindo independência.
            </li>
            <li>
              Um movimento aberto para quem quer tornar a cidade mais verde,
              acolhedora e sustentável. 🌱
            </li>
          </ul>
        </div>
      </div>
    </>
  );
}
